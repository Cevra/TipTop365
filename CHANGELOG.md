...
- **E9.4 `tiptop-e9.4-user-mgmt`:** user search/suspend/reactivate/impersonate. Suspension enforced once at `requireDbUser` (covers every authenticated route); impersonation mints a Firebase custom token whose `impersonatedBy` claim survives into the session cookie → permanent amber banner + audit; admins neither suspendable nor impersonatable. New `GET /api/auth/me`. +4 integration.
- **E9.5 `tiptop-e9.5-booking-mgmt`:** admin booking list + the three §8 interventions — no-show (§5 admin edge, full-refund effect), reassign (exact §6 reprice via `buildExactReprice`, extracted and now shared with first-accept — one reprice implementation), manual refund (escrow-sourced pre-settlement, revenue-reversal post-settlement; PSP + payments row + posting; all audited). +3 integration.
- **E5.5 `tiptop-e5.5-payout-runs`:** weekly payout runs — eligibility payable>0 ∧ IBAN on file ∧ no open dispute (skips reported per cleaner); BiH-format bank CSV (semicolons, comma decimals) flips draft→exported; mark-paid posts one idempotent §7 payout per row (double-click cannot double-drain); Friday cron `/api/jobs/prepare-payouts`; `/admin/payouts` UI. +2 integration.
- **E7.1 `tiptop-e7.1-contract-templates`:** 10 launch templates (fbih/fbih_student/rs/brcko temp_work + obrt self_bill × bs/en) with statutory references + day limits, DRAFT-watermarked, `lawyer_approved=false`, seeded idempotently. Editor creates NEW versions only; watermark re-injected if stripped (CLAUDE.md guardrail); approval toggle audited — E7.2's renderer drops the watermark only for approved versions. +3 integration.
- **E4.7 `tiptop-e4.7-price-adjustments`:** cleaner requests quarter-stepped extra hours (accepted/in_progress only, one open request); delta priced from the booking's own rate + snapshotted fee %; customer approval captures the delta before extra work (§5) with escrow posting and money-column increments — release later pays adjusted amounts with no extra wiring. Reject = money no-op. +3 integration.
